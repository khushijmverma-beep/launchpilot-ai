...
import { CheckSquare, ListChecks, MessageCircle, Send } from "lucide-react";
    ...
    <aside className="space-y-3 lg:sticky lg:top-5 lg:self-start">
      <section className="terminal-card p-3.5">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-sm font-semibold text-white">What to do</h2>
          </div>
        ...
        <ul className="mt-2.5 max-h-48 space-y-1.5 overflow-y-auto pr-0.5">
          {checklistItems.map((item, index) => {
            const isDone = Boolean(checked[index]);
            return (
              <li key={`${index}-${item}`}>
                <label className="flex cursor-pointer items-start gap-2 rounded px-1 py-0.5 hover:bg-white/5">
                  <input
                    type="checkbox"
                    checked={isDone}
                  ...
                  />
                  <span
                    className={`text-xs leading-5 ${isDone ? "text-lp-subtle line-through" : "text-lp-muted"}`}
                  >
                    {item}
                  </span>
                </label>
      ...
      <section className="terminal-card p-3.5">
        <div className="flex items-center gap-1.5">
          ...
          <h2 className="text-sm font-semibold text-white">Context Copilot</h2>
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {SUGGESTED_QUESTIONS.map((suggestion) => (
            <button
              key={suggestion}
              type="button"
              className="rounded border border-lp-border px-2 py-0.5 font-mono text-[10px] text-lp-subtle transition hover:border-white hover:text-white"
        ...
        <textarea
          className="mt-2 min-h-[3.25rem] w-full resize-none rounded border border-lp-border bg-black/40 p-2 text-xs text-white outline-none placeholder:text-lp-subtle focus:border-white"
        ...
        <button
          type="button"
          className="btn-primary mt-2 flex w-full items-center justify-center gap-1.5 py-2 text-xs"
        ...
        {answer && (
          <div className="mt-2 max-h-28 overflow-y-auto rounded border border-lp-border bg-black/30 p-2.5 text-xs leading-5 text-lp-muted">
            {answer}
          </div>
        )}

        {!answer && !loading && (
          <p className="mt-2 flex items-start gap-1.5 text-[10px] leading-4 text-lp-subtle">
            <CheckSquare className="mt-0.5 h-3 w-3 shrink-0" />
